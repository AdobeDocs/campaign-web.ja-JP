...
定期配信とトランザクション配信を使用する場合、カウントは親配信と子配信の両方に関連付けられます。
**R1** という名前の定期配信が、1 日目（RC1）、2 日目（RC2）、3 日目（RC3）に毎日実行されるように設定されている例を示します。
1 人の人物のみが、すべての子配信を複数回開いたとします。この場合、個別の子の定期配信では、各**[!UICONTROL 開封]**のカウントが 1 として表示されます。
ただし、同じユーザーがすべての配信をクリックしたので、親の定期配信では、**[!UICONTROL ユニーク開封]**&#x200B;も 1 となります。
