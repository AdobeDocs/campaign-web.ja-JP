...
>title="購読"
>abstract="この指標は、選択したチャネルに関し、サービスに対するすべての購読の合計と、正常に配信されたメッセージの合計数に対する、購読の割合を表示します。"


受信者は、E メールおよび SMS 通信をオプトインできます。

The **購読** 指標は、成功して配信されたメッセージの合計数に対する、チャネルごとの購読数の割合を示す、次の KPI を示します。
...
>title="送信済み"
>abstract="この指標は、ダイレクトメールチャネルに関し、配信準備段階で準備されたメッセージの合計数に対する、送信されたすべてのメッセージの合計と、プロバイダーに送信されたメッセージの割合を示します。"

準備段階では、ダイレクトメール抽出ファイルが生成されますが、受信者に関する情報（配信ログなど）は更新されません。  Campaign ユーザーが配信の送信を確認した場合に、配信のステータスが配信待ちから送信済みに移動したこと。 次に、配信が「完了」に設定されます。

準備されたメッセージの合計に比べて、送信されたメッセージの 100%を超えるアドレスがない、または不完全なアドレスが存在する可能性があるので、これを超えるメッセージは存在しない可能性があります。

The **送信済み** インジケーターは、ダイレクトメールチャネルに関する次の KPI を示します。
