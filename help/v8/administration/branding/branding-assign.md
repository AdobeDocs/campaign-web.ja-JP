...
1. また、パーソナライゼーションフィールドを使用してブランド情報を追加することもできます。**[!UICONTROL ブランディング]**&#x200B;に移動し、挿入するブランド関連のパーソナライゼーションブロック（ブランド名、ロゴ URL、web サイト URL、送信者情報など）を選択します。
...
リアルタイム（RT）実行インスタンスからコントロールインスタンスに同期されたトランザクション配信では、ルーティングやブランディングなどのプロパティはレプリケートされません。これらの同期された配信は、同じテンプレートから毎週生成され、コントロールインスタンスに配信指標を戻します。

これにより、コントロールインスタンスにはデフォルトのブランドが表示されます。メッセージ実行中に使用される実際のブランドとルーティング設定は、コントロールインスタンスのトランザクションメッセージテンプレートで定義されます。

トランザクションメッセージに使用されたブランドを確認するには：

1. リアルタイムに公開されたトランザクションテンプレートの内部名（例：`TransactionalMessaging4768`）を特定します。

   ![](assets/branding-transactional.png)

1. コントロールインスタンスの「**トランザクションメッセージテンプレート**」の下で、この内部名を検索します。
