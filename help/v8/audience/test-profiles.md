...
テストプロファイルは、配達確認を送信し、メッセージの内容と設定を検証するために使用されます。 これらのプロファイルは、メッセージを送信する前にパーソナライゼーションとレンダリングをプレビューおよびテストできる、追加の受信者です。 メッセージコンテンツをプレビューするときにテストプロファイルを選択し、配達確認をテストプロファイルに送信して、メッセージの内容と設定を制御および検証できます。
...
![ テストプロファイルリスト表示 ](assets/test-profile-list.png){zoomable="yes"}
...
* テストプロファイルを削除するには、**[!UICONTROL その他のアクション]**&#x200B;メニューから対応するオプションを選択します。
  ...
  ![ テストプロファイルオプションを削除 ](assets/test-profile-list-delete.png){zoomable="yes"}
...
ここから、フォルダーやサブフォルダーを参照、作成および管理したり、関連する権限を確認したりできます。 [フォルダーの作成方法を学ぶ](../get-started/permissions.md#folders)

![ テストプロファイルフォルダー表示 ](assets/test-profiles-folders.png){zoomable="yes"}

**[!UICONTROL エクスプローラー]** ビューでは、テストプロファイルのフィルタリング、削除、編集、および [ 作成 ](#create-test-profile) を行うこともできます。
...
1. 「**[!UICONTROL テストプロファイルを作成]**」ボタンをクリックします。
   ...
   ![ テストプロファイルを作成ボタン ](assets/test-profile-create.png){zoomable="yes"}
...
1. 「**[!UICONTROL 連絡先情報]**」セクションに、メールアドレスと他の関連データを入力します。メールアドレスは、テストプロファイルラベルの後の括弧内に表示されます。
   ...
   ![ 連絡先情報セクション ](assets/test-profile-address.png){zoomable="yes"}
