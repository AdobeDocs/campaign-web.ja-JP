...
ブランドに関連しなくなったフラグメントをフラグメントリストから削除できます。これを行うには、目的のフラグメントの横にある「**[!UICONTROL その他のアクション]**」ボタンをクリックし、「**[!UICONTROL アーカイブ]**」を選択します。フラグメントはフラグメントリストから削除され、ユーザーは今後のメールやテンプレートでフラグメントを使用できなくなります。
...
>[!NOTE]
>
>コンテンツで使用されているフラグメントをアーカイブした場合、そのコンテンツは影響を受けません。
