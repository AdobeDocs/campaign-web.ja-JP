...
audience: end-user
title: 生成コンテンツ
description: AI を使用してテキストコンテンツエクスペリエンスを生成する方法について学ぶ
...
></br>
>
>Adobe Campaign Web の AI を使用する前に、[ユーザー契約](https://www.adobe.com/jp/legal/licenses-terms/adobe-dx-gen-ai-user-guidelines.html){target="_blank"}に同意する必要があります。詳しくは、アドビ担当者にお問い合わせください。

Adobe Campaign Web の AI を使用して、オーディエンスの共感を得られる魅力的なテキストを生成します。メールのコピーを拡充させたり、説得力のあるランディングページのコンテンツを作成したり、プッシュ通知メッセージを作成したり、SMS テキストを作成したりする場合でも、AI は明確で効果的なコミュニケーションの提供に役立ちます。

## メールとランディングページの場合 {#email-web-channels}

AI を使用して、メール配信やランディングページに高品質なテキストコンテンツを生成します。この機能により、デジタルタッチポイントをまたいでオーディエンスとつながる、魅力的でブランドに即したメッセージを作成できます。

### アクセスと設定 {#access-configure}

AI を使用してテキストコンテンツの生成を開始する前に、配信を設定し、コンテンツエディターにアクセスする必要があります。ワークスペースを準備し、コンテンツを生成パネルを開くには、次の手順に従います。
...
1. 「**[!UICONTROL テキストコンポーネント]**」を選択して特定のコンテンツをターゲットに設定し、**[!UICONTROL コンテンツを生成]**&#x200B;メニューにアクセスします。

   ![Adobe Campaign Web でのテキストコンポーネントの選択を示すスクリーンショット](assets/text-genai-1.png){zoomable="yes"}

### テキストを生成 {#generate-content}

AI を使用して明確なプロンプトを作成し、設定を微調整し、カスタマイズされたテキストを生成して、メッセージがブランドやコミュニケーションの目標に沿っていることを確認する方法について説明します。
...
1. 「**[!UICONTROL テキスト設定]**」オプションを使用してプロンプトを調整します。
   ...
   * **[!UICONTROL トーン]**：メールのトーンは、オーディエンスの共感を呼びます。 情報の提供や遊び心、説得力のどの要素を加えたい場合であっても、AI はメッセージを適切に調整します。
   * **テキストの長さ**：スライダーを使用して、テキストの目的の長さを選択します。

     ![Adobe Campaign web のテキスト設定オプションを示すスクリーンショット](assets/text-genai-4.png){zoomable="yes"}

1. **[!UICONTROL 参照コンテンツ]**&#x200B;メニューから、「**[!UICONTROL ファイルをアップロード]**」をクリックして、AI に追加のコンテキストを提供できるコンテンツを含むブランドアセットを追加するか、以前にアップロードしたものを選択します。
...
AI を使用して、プッシュ通知や SMS メッセージ用の魅力的なテキストコンテンツを生成すると、すべてのモバイルタッチポイントをまたいでオーディエンスとつながる魅力的なモバイルコミュニケーションを作成できます。

### アクセスと設定 {#mobile-access-configure}

モバイルチャネル用に AI を使用してテキストの生成を開始する前に、配信を設定し、「コンテンツを生成」にアクセスする必要があります。
...
1. **[!UICONTROL 「コンテンツを生成」を表示]**&#x200B;メニューにアクセスします。

   ![「コンテンツを生成」を表示メニューを示すスクリーンショット](assets/sms-genai-1.png){zoomable="yes"}

### テキストを生成 {#mobile-generate-content}

「コンテンツを生成」にアクセスしたら、生成設定を指定して、ブランドと配信の目標に合わせたモバイルコンテンツを作成できます。テキストパラメーターをカスタマイズし、ブランドアセットを追加し、関連するバリエーションの生成で AI をガイドするプロンプトを指定します。
...
1. 「**[!UICONTROL プロンプト]**」フィールドに生成する内容を記述して、コンテンツを微調整します。
   ...
   ![プロンプトフィールドとオプションが表示された「コンテンツを生成」](assets/sms-genai-2.png){zoomable="yes"}
...
1. 「**[!UICONTROL テキスト設定]**」オプションを使用してプロンプトを調整します。
   ...
   * **[!UICONTROL コミュニケーション戦略]**：生成テキストに最適なコミュニケーションスタイルを選択します。
   * **[!UICONTROL 言語]**：生成されたコンテンツの言語を選択します。
   * **[!UICONTROL トーン]**：トーンは、オーディエンスの共感を呼ぶ必要があります。 情報の提供や遊び心、説得力のどの要素を加えたい場合であっても、AI はメッセージを適切に調整します。
...
1. **[!UICONTROL プレビュー]**&#x200B;ウィンドウ内の「**[!UICONTROL 絞り込み]**」オプションに移動して、追加のカスタマイズ機能にアクセスします。
   ...
   * **[!UICONTROL よりシンプルな言葉を使用]**：AI を使用して言葉をわかりやすく編集／調整し、幅広いオーディエンスがアクセスできるようにします。
