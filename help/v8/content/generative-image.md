...
audience: end-user
title: 生成コンテンツ
description: AI を使用して画像を生成する方法について学ぶ
...
Adobe Campaign Web で AI を使用して、メール、ランディングページ、プッシュ通知をまたいでメッセージを強化する魅力的なビジュアルコンテンツを作成します。AI は、画像の生成と最適化に役立ち、コンテンツが視覚的に魅力的で、ブランドに即したものなるようにします。

## メールとランディングページの場合 {#email-web-channels}

AI を使用して、メール配信やランディングページに完全なビジュアルエクスペリエンスを生成します。この機能により、デジタルタッチポイントをまたいでオーディエンスの共感を得られる、ブランドに即した注目を集める画像を作成できます。

### アクセスと設定 {#access-configure}

AI を使用して画像の生成を開始するには、まず配信を設定して、コンテンツエディターを開きます。ワークスペースを準備し、コンテンツを生成パネルにアクセスするには、次の手順に従います。
...
AI を使用して効果的なプロンプトを作成し、画像設定を指定して、視覚的に魅力的な画像を生成する方法について説明します。縦横比、視覚強度、照明などのパラメーターをカスタマイズして、ブランドやキャンペーンの目標に一致する画像を作成します。

1. AI を使用する「**[!UICONTROL 参照スタイル]**」オプションを有効にして、選択したコンテンツに基づいて新しいコンテンツをパーソナライズします。
...
AI を使用すると、プッシュ通知用の魅力的な画像を生成できるので、注目を集め、オーディエンスの共感を得る、視覚的に魅力的なモバイルコミュニケーションを作成できます。

### アクセスと設定 {#mobile-access-configure}

AI を使用してプッシュ通知用の画像の生成を開始するには、まず配信を設定して、コンテンツを生成を開きます。

1. プッシュ通知配信を作成および設定したら、「**[!UICONTROL コンテンツを編集]**」をクリックします。 [詳細情報](../push/create-push.md)

1. **[!UICONTROL 「コンテンツを生成」を表示]**&#x200B;メニューにアクセスします。

   ![「コンテンツを生成」を表示メニューを示すスクリーンショット](assets/push-img-1.png){zoomable="yes"}

### 画像を生成 {#mobile-generate-content}

コンテンツを生成にアクセスしたら、生成設定を調整して、ブランドに一致する画像を作成し、プッシュ通知の目標をサポートできます。プロンプトと画像のパラメーターを設定して、モバイルディスプレイに最適化されたビジュアルを生成します。
...
1. 「**[!UICONTROL プロンプト]**」フィールドに生成する内容を記述して、コンテンツを微調整します。
   ...
   ![プロンプトフィールドとオプションが表示された「コンテンツを生成」](assets/push-img-2.png){zoomable="yes"}
