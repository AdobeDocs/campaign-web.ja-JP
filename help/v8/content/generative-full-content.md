...
audience: end-user
title: 生成コンテンツ
description: Journey Optimizer で AI を使用して完全なコンテンツエクスペリエンスを生成する方法について説明します。
...
Adobe Campaign Web で AI を使用し、メール、ランディングページ、プッシュ通知チャネル全体にわたって完全なコンテンツエクスペリエンスを生成します。AI は、オーディエンスの共感を呼ぶ包括的なコンテンツを作成することで、配信の影響を最適化できるよう支援します。

## メールとランディングページの場合 {#email-web-channels}

AI を使用してテキストと画像の両方を生成し、メール配信とランディングページ向けに完全なコンテンツエクスペリエンスを生成します。この堅牢な機能により、すべてのデジタルタッチポイントをまたいでオーディエンスとつながる、魅力的でブランドに即したコンテンツを作成できます。

### アクセスと設定 {#access-configure}

AI を使用してコンテンツの作成を開始する前に、配信を設定し、コンテンツエディターを開く必要があります。ワークスペースを準備し、コンテンツを生成パネルにアクセスするには、次の手順に従います。
...
1. 必要に応じてレイアウトをパーソナライズし、**[!UICONTROL コンテンツを生成]**&#x200B;メニューにアクセスします。
   ...
   ![ブランド選択とプロンプトフィールドを示す、コンテンツを生成パネル](assets/full-email-1.png){zoomable="yes"}

### 完全なコンテンツを生成 {#generate-content}

「コンテンツを生成」を開き、生成設定を指定して、ブランドやキャンペーンの目標に沿ったコンテンツを作成できるようになりました。テキストと画像のパラメーターをカスタマイズし、ブランドアセットを追加し、オーディエンスに関連するバリエーションの生成で AI をガイドするプロンプトを指定します。
...
「コンテンツを生成」はモバイルプッシュ通知のコンテンツ生成もサポートしており、モバイルアプリの魅力的なタイトル、メッセージ、画像を作成できます。これにより、モバイルを含むすべての顧客タッチポイントで一貫性のある高品質のコミュニケーションを維持できます。

### アクセスと設定 {#mobile-access-configure}

プッシュ通知に AI を使用してコンテンツを生成するには、まずプッシュ配信を設定し、コンテンツエディターを開きます。配信の準備、および「コンテンツを生成」ツールへのアクセスを実行するには、次の手順に従います。
...
1. **[!UICONTROL 「コンテンツを生成」を表示]**&#x200B;メニューにアクセスします。
   ...
   ![「コンテンツを生成」を表示メニューのスクリーンショット](assets/push-genai-1.png){zoomable="yes"}

### 完全なコンテンツを生成 {#mobile-generate-content}

プッシュ通知用のコンテンツを生成にアクセスしたら、生成設定を指定して、魅力的なモバイルコンテンツを作成できます。テキストと画像の環境設定を定義し、ブランドアセットを選択し、プロンプトを使用してモバイルユーザーに関与するプッシュ通知のバリエーションを生成します。
...
1. 「**[!UICONTROL プロンプト]**」フィールドに生成する内容を記述して、コンテンツを微調整します。
   ...
   ![プロンプトフィールドとオプションをが表示された、「コンテンツを生成」](assets/push-genai-2.png){zoomable="yes"}
...
1. 「**[!UICONTROL テキスト設定]**」オプションを使用してプロンプトを調整します。
   ...
   * **[!UICONTROL コミュニケーション戦略]**：生成テキストに最適なコミュニケーションスタイルを選択します。
   * **[!UICONTROL 言語]**：生成されたコンテンツの言語を選択します。
   * **[!UICONTROL トーン]**：プッシュ通知のトーンは、オーディエンスの共感を呼ぶものでなければなりません。 情報の提供や遊び心、説得力のどの要素を加えたい場合であっても、AI はメッセージを適切に調整します。
...
1. **[!UICONTROL 参照コンテンツ]**&#x200B;メニューから、「**[!UICONTROL ファイルをアップロード]**」をクリックして、AI に追加のコンテキストを提供できるコンテンツを含むブランドアセットを追加するか、以前にアップロードしたファイルを選択します。
...
AI を使用して、完全なメールコンテンツ、テキスト、画像を生成する方法について説明します。
