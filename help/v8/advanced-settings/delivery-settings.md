...
* **[!UICONTROL 送信者のアドレス]**
  ...
  SMPP 仕様では、このフィールドの文字数が 21 文字に制限されていますが、プロバイダーによっては、これより長い値を使用できる場合があります。また、一部の国では、非常に厳しい制限（長さ、コンテンツ、許可されている文字など）が適用される場合があるので、ここに配置するコンテンツが合法であることを再確認する必要があります。 パーソナライズされたフィールドを使用する際は、特に注意する必要があります。


  このオプションのフィールドを使用すると、送信者アドレス（oADC）を上書きできます。 その内容は、SUBMIT_SM PDU の *source_addr* フィールドに配置されます。

  SMPP の仕様では、このフィールドを 21 文字に制限していますが、一部のプロバイダーはより長い値をサポートする場合があります。 特定の国では（長さ、コンテンツ、許可されている文字などに関して）送信者のアドレスに対して厳しい規制が課されていることに注意してください。そのため、入力が地域の要件に準拠していることを常に確認してください。 パーソナライズされたフィールドを使用する場合は、特に注意が必要です。
