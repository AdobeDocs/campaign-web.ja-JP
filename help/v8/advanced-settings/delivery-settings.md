...
これらの設定は、 **技術的な配信パラメーター** 電子メールテンプレートで定義される。 特定の配信について、これらのいずれかを変更する場合は、注意して作業を進めてください。
...
タイポロジとは、メッセージの分析段階で実行される一連の&#x200B;**タイポロジルール**&#x200B;です。これらを使用すると、特定の要素（購読解除リンクや件名行など）や、対象とするターゲットから特定のグループ（非購読者、競合他社、非ロイヤルティ顧客など）を除外するフィルタリングルールがメールに必ず含まれていることを確認できます。
...
「**重み付けタイプ**」フィールドでは、次の 3 つのオプションを使用できます。 （オプションに応じた数式がありません…）
...
このセクションでは、 **ターゲットマッピング** Adobe Campaign v8 コンソールで定義されます。 Adobe Campaign で提供される受信者テーブル以外の受信者テーブルを使用する場合は、ターゲットマッピングの作成が必要です。
...
ソフトまたは無視のエラーによって一時的に配信できなかったメッセージは、自動再試行の対象となります。デフォルトでは、配信の初日に最低 1 時間の間隔をおいて 24 時間に 5 回の再試行がスケジュールされます。その後は、「有効性」タブで指定される配信期限が来るまで、1 日 1 回の再試行がスケジュールされます。
...
**トラッキングの有効制限**：このオプションは、トラッキングが URL でアクティブ化される期間を定義します。
