...
配信設定は次のとおりです。 **技術的な配信パラメーター** 配信テンプレートで定義された 配信ごとにオーバーロードする可能性があります。 これらの設定は、 **設定** ボタンが使用可能になります。
...
タイポロジを使用すると、配信に常に特定の要素（購読解除リンクや件名）やフィルタールールを含めて、意図したターゲット（購読解除、競合者、非ロイヤルティ顧客など）からグループを除外できます。
...
統合された **[!UICONTROL Routing]** デフォルトでは、外部アカウントが提供されます。 アプリケーションが配信を送信するための技術的なパラメーターが含まれます。
...
E メールの場合は、 **[!UICONTROL メールフォーマット]** 送信されるメッセージの数を次に示します。

* **[!UICONTROL 受信者の環境設定を使用]** （デフォルトモード）:E メールフォーマットは、受信者プロファイルに保存されたデータに従って定義されます。 受信者が特定の形式で E メールを受信したい場合、送信される形式がその受信者に該当します。 このフィールドに何も入力されていない場合は、マルチパート/オルタナティブの E メールが送信されます（以下を参照）。

* **[!UICONTROL 受信者のメールクライアントに最適なフォーマットを選択させる]**:E メールには、テキストとHTMLの両方の形式が含まれます。 受信時に表示されるメッセージ形式は、受信者のメールソフトウェアの設定に応じて切り替わります（マルチパート／オルタナティブ）。

  >[!IMPORTANT]
  >
  >このオプションを指定すると、両方のバージョンのドキュメントがメッセージに含められます。したがって、E メールのサイズが大きくなるので、配信率に影響します。

* **[!UICONTROL すべてのメッセージをテキスト形式で送信]**:E メールはテキスト形式で送信されます。 HTMLの形式は送信されませんが、受信者が E メールをクリックした場合にのみ表示されるミラーページに使用されます。
...
E メール配信の準備中に警告が生成された場合は、配信を設定して、実行する必要があるかどうかを定義できます。 デフォルトでは、分析フェーズの最後に、ユーザーが E メールの送信を確認する必要があります。 **手動** 検証。
...
### ミラーページ管理（E メールチャネル） {#mirror}

ミラーページは、web ブラウザーでオンラインアクセス可能な HTML ページです。そのコンテンツはメールと同一です。デフォルトでは、リンクが E メールのコンテンツに挿入された場合、ミラーページが生成されます。

デフォルトモードのほかに、次のオプションも選択できます。

* **[!UICONTROL ミラーページを強制的に生成]**：このモードを使用すると、E メールにミラーページへのリンクが挿入されていない場合でもミラーページを生成できます。
* **[!UICONTROL ミラーページを生成しない]**:E メールにリンクが存在する場合でもミラーページが生成されないようにするには、このモードを使用します。
