...
>title="リソースの有効期間"
>abstract="「有効制限」フィールドは、ミラーページや画像など、アップロードされたリソースに対して使用されます。 以下のリソースは、限られた期間有効です。制限に達すると、リソースは使用できなくなります。"
...
* **[!UICONTROL メッセージ識別子のみを使用してアクセス可能なミラーページを生成します]**:e メールコンテンツにミラーページのリンクが存在しない場合は、クライアントコンソールから、配信ログウィンドウでミラーページのコンテンツにアクセスできるようにするには、このオプションを使用します。
