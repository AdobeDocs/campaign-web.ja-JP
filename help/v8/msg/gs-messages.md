...
Adobe Campaign v8 には、次の配信チャネルが付属しています。E メール、SMS、モバイルアプリ。
...
## 配信の削除 {#delivery-delete}

配信は、左側のレールのメインの配信エントリまたはキャンペーンの配信リストから、配信リストから削除されます。
...
![配信リストからの配信の削除](assets/delete-delivery-from-list.png)
...
1. 配信アクティビティを選択します。
1. 次をクリック：  **[!UICONTROL 削除]** アイコンをクリックします。
1. 削除の確認. 配信に子ノードがある場合、それらも削除するか、保持するかを選択できます。
