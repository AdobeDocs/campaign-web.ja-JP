...
>title="プロファイルエンリッチメントを有効にする"
>abstract="プロファイルエンリッチメントのフィールドを選択します。 紐付けキーとして選択できるのは、インデックス付きのフィールドのみです。サポートされていないフィールドは、グレー表示されます。"
...
配信の送信をスケジュールするには、次の手順に従います。

1. 配信を開き、**[!UICONTROL スケジュール]** セクションを参照します。
1. **[!UICONTROL スケジュールを有効にする]**&#x200B;切替スイッチをオンにしてアクティブ化します。
1. 「**[!UICONTROL お問い合わせ日]**」フィールドで、カレンダーアイコンをクリックし、送信する日時を設定します。
1. 「**[!UICONTROL タイムゾーン]**」フィールドで、ドロップダウンメニューを使用して、目的のタイムゾーンを選択します。

配信を送信すると、実際の送信は、定義した連絡日に開始されます。

![配信のスケジュールインターフェイスを示すスクリーンショット](assets/schedule.png){zoomable="yes"}

デフォルトでは、「**[!UICONTROL 送信前に確認を有効にする]**」オプションが有効になっています。このオプションでは、配信がスケジュールされた日付に送信される前に送信を確認する必要があります。スケジュールされた日時に配信を自動的に送信する必要がある場合は、このオプションを無効にできます。
