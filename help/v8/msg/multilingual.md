...
1. 「**言語を追加**」ドロップダウンから、追加する言語を選択し、確定します。 プッシュ通知の場合は、[CSV ファイルをアップロード](#csv-upload)して、すべての言語のバリアントを一度にインポートすることもできます。
...
## CSV から言語バリアントをインポート（プッシュ通知） {#csv-upload}

多言語コンテンツを含む CSV ファイルをアップロードして、すべての言語バリアントをすばやく作成できるようになりました。この機能は、コンテンツをオフラインで準備し、一括インポートできるようにすることで、多言語キャンペーンの作成を合理化します。

* **効率**：複数の言語とそのコンテンツを 1 回の操作で追加します
* **一貫性**：すべての言語バリアントをまたいで統一されたメッセージを確保します
* **コラボレーション**：コンテンツチームが使い慣れたスプレッドシートツールで翻訳を準備できるようにします
* **一括管理**：大量の言語バリアントを簡単に管理および更新します

### 前提条件 {#csv-best-practices}

CSV のインポートを確実に成功させるには、次のベストプラクティスに従ってください。

* **正確な列構造を使用**：一部を空白のままに残しても、14 列すべてが CSV ファイルに含まれている必要があります。列が見つからないと、インポートエラーが発生します。別の順序を使用することもできますが、すべての列が存在する必要があります。
* **列名が正確に一致する**：列名では大文字と小文字が区別されます。`Title` ではなく `title`、`Bbadge` ではなく `badge`、`Locale` ではなく `locale` を使用してください。
* **小文字のロケールコードを使用**：ロケールコードの形式は、`en_us`、`fr_fr`、`de_de`（アンダースコア付きの小文字）です。`en_US` や `en-us` ではありません。
* **必須列を入力**：`locale` 列と `language` 列には、各行の値を含める必要があります。値が空の場合、インポートエラーが発生します。
* **ロケールを一意に保つ**：各ロケールコードは、CSV ファイルで 1 回だけ使用する必要があります。重複したロケールは拒否されます。
* **UTF-8 として保存**：国際文字を正しくサポートするために、常に UTF-8 エンコーディングを使用して CSV ファイルを保存します。
* **引用コンマを含むコンテンツ**：タイトルまたはメッセージ本文にコンマが含まれる場合、次のようにフィールド全体を二重引用符で囲みます：`"Hello, welcome!"`。
* **数値を正しく使用する**：フラグ列（isContentAvailable、isMutableContent、silentPush）の場合は、true に `1` を使用し、false に `0` を使用するか、デフォルトで空白のままにします。
* **JSON 形式を検証**：customFields 列を使用する場合は、JSON が正しくフォーマットされていることを確認してください：`{"key":"value"}`（引用符と角括弧が正しいこと）。
* **最小限のデータでテストを開始**：大きなファイルを作成する前に、単純な 2 ～ 3 言語の CSV から形式を検証します。

>[!NOTE]
>
>この手順について詳しくは、[この節](#csv-columns)を参照してください。

### CSV ファイルをインポート {#csv-steps}

CSV ファイルから言語バリアントを読み込むには、次の手順に従います。

1. 配信コンテンツエディターから、「**[!UICONTROL 言語を追加]**」をクリックします。

   ![プッシュ通知コンテンツエディターの「言語を追加」ボタンを示すスクリーンショット](assets/multilingual-csv.png){zoomable="yes"}

1. CSV ファイルをアップロード領域にドラッグ &amp; ドロップして選択するか、クリックしてコンピュータを参照します。

   システムによってファイル形式とコンテンツが検証されます。検証が失敗した場合、エラーメッセージは、どの列またはデータが正しくないかを示します。CSV ファイルの問題を修正し、再度アップロードします。[こちら](#csv-troubleshooting)を参照してください。

   ![インポートされたすべての言語で CSV 検証が成功したことを示すスクリーンショット](assets//multilingual-csv2.png){zoomable="yes"}

1. 言語バリアントパネルでインポートされたコンテンツを確認して、すべての翻訳が正しく読み込まれることを確認します。

   ![インポートされた多言語コンテンツのバリアントのプレビューを示すスクリーンショット](assets/multilingual-csv3.png){zoomable="yes"}
...
>[!NOTE]
>
>別の順序を使用することもできますが、すべての列が存在する必要があります。ベストプラクティスについては、この[節](#csv-best-practices)を参照してください。

1. **title**：通知タイトル（必須）
1. **messageBody**：通知メッセージ本文（必須）
1. **sound**：音声ファイル名（例：`default`、`custom_sound.mp3`） – デフォルトでは空白のままにします
1. **badge**：アプリアイコンに表示するバッジ番号（iOS） – 番号のみを使用
1. **deeplinkURI**：通知がタップされたときに開くディープリンク URL。使用しない場合は空白のままにします
1. **category**：カスタムアクションの通知カテゴリ ID （iOS） – 使用しない場合は空白のままにします
1. **iosMediaAttachmentURL**：iOS 通知のメディア添付ファイルのURL – 使用しない場合は空白のままにします
1. **androidMediaAttachmentURL**：Android 通知のメディア添付ファイルの URL – 使用しない場合は空白のままにします
1. **isContentAvailable**：コンテンツ利用可能フラグ（iOS）- true には `1` を使用し、false には `0` を使用し、デフォルト（0）では空白のままにします
1. **isMutableContent**：可変コンテンツフラグ（iOS）- true には `1` を使用し、false には `0` を使用し、デフォルト（0）では空白のままにします
1. **customFields**: JSON 形式のカスタムデータ（例：`{"key1":"value1","key2":"value2"}`）– 使用しない場合は空白のままにします
1. **ロケール**：言語コード（必須）- 例：`en_us`、`fr_fr`、`de_de` - **必須、行ごとに一意である必要があります**
1. **言語**：言語名（必須）- 例：`English-United States`、`French-France` - **必須**
1. **silentPush**: サイレントプッシュフラグ – サイレントプッシュには `1` を使用し、通常のプッシュには `0` を使用し、デフォルト（0）では空白のままにします

### CSV ファイルの例 {#csv-examples}

以下は、必須フィールドを使用した基本的な例です。
...
>[!NOTE]
>
>カルーセルまたはアクションボタンを含むリッチプッシュ通知の場合、Campaign では CSV インポートとは異なる設定方法を使用します。基本的な多言語コンテンツを読み込んだ後に、配信エディターでリッチプッシュコンテンツを直接設定できます。

### CSV ファイル内のパーソナライズ機能 {#csv-personalization}
...
配信中、Campaign はこれらのプレースホルダーを実際の受信者データに置き換えます。

### トラブルシューティング {#csv-troubleshooting}

| エラー | 原因 | ソリューション |
|-------|-------|----------|
| 必須の列が欠落しています | CSV ファイルに 14 列がすべて含まれていない | CSV に 14 列がすべて上記の順序になっていることを確認します。未使用の列には空の値を使用します。 |
| 無効なロケール値 / 言語値 | ロケール列または言語列が空です | ロケール列と言語列の各行に値が必要です |
| 重複したロケール | 同じロケールコードが複数回表示される | 各ロケール値は一意である必要があります – 重複する行を削除します |
| ファイルエンコーディングの問題 | CSV ファイルで互換性のないエンコーディングが使用されています | UTF-8 エンコーディングを使用した CSV ファイルを保存 |
| 列の不一致 | 行の列数がヘッダーの列数と異なります | すべての行にヘッダーと一致する列が正確に 14 個あることを確認します |
| 無効な数値 | badge、isContentAvailable、isMutableContent、または silentPush に数値以外の値が含まれています | フラグには 0 または 1 の数字のみを使用するか、デフォルトでは空白のままにします |
| JSON の形式が正しくありません | customFields 列に無効な JSON が含まれています | JSON 構文が正しいことを確認します。`{"key":"value"}`または、空白のままにします |
| 列名の大文字と小文字が一致していません | 列名が正確に一致していません | 列名では大文字と小文字が区別されます。上記の名前と完全に一致する名前を使用します（例：`Badge` や `BADGE` ではなく `badge` を使用します）。 |

>ベストプラクティスについて詳しくは、この[セクション](#csv-best-practices)を参照してください。列の構造について詳しくは、この[セクション](#csv-columns)を参照してください。
