...
1. ルールビルダーが表示されます。条件の作成に活用できる&#x200B;**プロファイル**、**メッセージ**&#x200B;および&#x200B;**配信**&#x200B;属性が提供されます。

   また、**条件**&#x200B;メニューには、バリアントを簡単に作成するためにキャンバスに追加できる事前定義済みの条件が用意されています。これらを使用するには、「**追加**」ボタンをクリックし、`<` と `>` の記号の間にある要素を目的の要素に置き換えます。`<value>` オブジェクトに指定する値は、引用符で囲みます。

   ![](assets/condition-syntax.png){zoomable=&quot;yes&quot;}{width="800" align="center"}

1. メッセージの最初のバリアントの条件が整ったら、「**[!UICONTROL 確認]**」をクリックします。この例では、言語が「フランス語」の受信者をターゲティングとするルールを作成します。
