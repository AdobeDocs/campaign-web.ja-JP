...
>[!NOTE]
>
>オーディエンスや定義済みフィルターなどのルールが適用されているクライアントコンソールで作成したオブジェクトにアクセスすると、「**[!UICONTROL ターゲットを絞り込み]** セクションが表示される場合があります。 これは、ルールのターゲットを絞り込むために、追加のパラメーターが設定されていることを意味します。 これらのパラメーターの変更は、コンソールでのみ可能です。
>
