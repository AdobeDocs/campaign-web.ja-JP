...
* **コンポーネントのフィルタリング**（カスタム条件、オーディエンスを選択、定義済みフィルター）を使用すると、独自のルールを作成したり、オーディエンスまたは定義済みフィルターを選択してクエリを絞り込んだりできます。クエリの開始時およびドット切り替え時に追加されます。 [詳しくは、コンポーネントフィルタリングの操作方法を参照してください](#filtering)

  例： *「スポーツ」ニュースレターを購読した受信者*. *ニューヨークに住む受信者*、*サンフランシスコに住む受信者*

  ![](assets/query-add-component.png){zoomable=&quot;yes&quot;}

* **グループオペレーター** (AND、OR、EXCEPT) を使用すると、ダイアグラム内のフィルタリングコンポーネントをグループ化できます。 これらは、フィルタリングコンポーネントの前に既存のトランジションに追加されます。 [詳しくは、演算子の操作方法を参照してください](#filtering)
...
1. 「**属性**」フィールドで、条件の作成に活用するデータベースから属性を選択します。属性リストには、リンクされたテーブルの属性を含む、Campaign データベースのすべての属性が含まれます。
   ...
   >[!NOTE]
   >
   >The **式を編集** ボタンをクリックすると、Campaign Web 式エディターを利用して、データベースのフィールドとヘルパー関数を使用して式を手動で定義できます。 [式の編集方法を学ぶ](expression-editor.md)
...
1. 「**値**」フィールドで、期待値を定義します。また、Campaign web 式エディターを利用して、データベースのフィールドとヘルパー関数を使用して式を手動で定義することもできます。それには、「**式を編集**」ボタンをクリックします。[詳しくは、式の編集方法を参照してください](expression-editor.md)

   *21 歳以上のすべてのプロファイルを返すクエリの例：*

   ![](assets/query-custom-condition.png){zoomable=&quot;yes&quot;}

   日付タイプの属性の場合、事前定義済みの値は **[!UICONTROL プリセット]** オプション。
...
この例では、2 つ目のトランジションに新しいオーディエンスタイプのフィルタリングコンポーネントを追加しました。このコンポーネントは、 **および** 演算子を使用する場合、クエリ結果には、「マドリディアン」という事前定義済みのフィルター AND が「割引ハンター」オーディエンスに属する受信者が含まれます。
