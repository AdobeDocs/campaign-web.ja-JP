...
1. 適用する演算子をドロップダウンリストから選択します。 様々な演算子を使用できます。ドロップダウンリストで使用できる演算子は、選択した属性のデータタイプによって異なります。
...
* **OR（和集合）**：アウトバウンドトランジションで、少なくとも 1 つのフィルタリングコンポーネントの結果が含まれます。
* **EXCEPT （除外）**：アウトバウンドトランジションのすべてのフィルタリングコンポーネントから結果を除外します。
