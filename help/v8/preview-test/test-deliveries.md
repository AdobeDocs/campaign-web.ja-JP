...
プロファイルの代用を使用すると、[!DNL Adobe Campaign] データベースの既存プロファイルのデータを表示しながら、特定のメールアドレスまたは電話番号にテストメッセージを送信できます。手順は次のとおりです。
...
## サブスクライバーへのテストの送信 {#subscribers}

プッシュ通知を使用する場合、テスト配信はアプリのサブスクライバーにのみ送信できます。これらのサブスクライバーを選択するには、次の手順に従います。
