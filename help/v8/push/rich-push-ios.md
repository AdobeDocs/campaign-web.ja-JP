...
>[!IMPORTANT]
>
>リッチプッシュ通知をデザインする前に、V2 コネクタを設定する必要があります。 手順について詳しくは、[このページ](https://experienceleague.adobe.com/ja/docs/campaign-classic/using/sending-messages/sending-push-notifications/configure-the-mobile-app/configuring-the-mobile-application)を参照してください。
...
1. 「**[!UICONTROL タイトル]**」フィールドで、通知センターから使用可能な通知のリストに表示するタイトルのラベルを入力します。
   ...
   このフィールドは、iOS通知ペイロードの **title** パラメーターの値を定義します。
...
1. プッシュ通知をさらにパーソナライズするには、プッシュ通知に追加する **[!UICONTROL 起動画像]** URL を選択します。
...
1. **[!UICONTROL アプリケーション変数]**&#x200B;メニューから、**[!UICONTROL アプリケーション変数]**&#x200B;が自動的に追加されます。これにより、ユーザーが通知を有効化したときに表示される特定のアプリケーション画面の設定など、通知動作を定義できます。

メッセージコンテンツを定義したら、テストサブスクライバーを使用して、メッセージをプレビューしテストします。
...
1. メッセージを作成するには、「**[!UICONTROL タイトル]**」、「**[!UICONTROL 展開されたタイトル]**」、「**[!UICONTROL メッセージ]**」、「**[!UICONTROL 展開されたメッセージ]** フィールドにテキストを入力します。
   ...
   折りたたまれたビューに「**[!UICONTROL メッセージ]**」テキストが表示され、通知を展開すると「**[!UICONTROL 展開されたメッセージ]**」が表示されます。
...
1. 「**[!UICONTROL 画像を追加]**」をクリックし、**[!UICONTROL 画像]** URL、**[!UICONTROL テキスト]**、**[!UICONTROL アクション URI]** を入力します。
...
1. **[!UICONTROL タイマー期間]**&#x200B;を秒単位で設定するか、**[!UICONTROL タイマー終了タイムスタンプ]**&#x200B;を特定のエポックタイムスタンプに設定します。
   ...
   ![ タイマー時間設定画面 ](assets/rich_push_ios_timer_3.png)
...
| パラメーター | 説明 |
|-----------|-------------|
| **[!UICONTROL 重大なアラートモード]** | ユーザーの電話がフォーカスモードに設定されている場合やミュートされている場合でも、通知にサウンドを追加するには、このオプションを有効にします。 |
...
| パラメーター | 説明 |
|-----------|-------------|
| **[!UICONTROL 通知の背景色]** | 16 進数のカラーコードを使用して、通知の背景の色を設定します。 |
| **[!UICONTROL タイトルの色]** | 16 進数のカラーコードを使用してタイトルの色を設定します。 |
| **[!UICONTROL メッセージテキストの色]** | 16 進数のカラーコードを使用してテキストの色を設定します。 |
...
| **[!UICONTROL 割り込みレベル]** | <ul><li>**アクティブ**：デフォルトでは、通知が直ちに表示され、画面が点灯し、サウンドを再生できます。通知はフォーカスモードを妨げません。</li><li>**パッシブ**：画面を点灯させたり、サウンドを再生したりせずに、システムは通知リストに通知を追加します。通知はフォーカスモードを妨げません。</li><li>**時間依存**：システムにより通知がすぐに表示され、画面が点灯し、サウンドを再生でき、フォーカスモードで通知が再生されます。 このレベルの操作には、Appleから特別な権限は必要ありません。</li><li>**重大**：システムにより通知が直ちに表示され、画面が点灯し、ミュートスイッチまたはフォーカスモードがバイパスされます。このレベルには、Appleからの特別な権限が必要です。</li></ul> |
