...
1. メール設定画面の上部にある「**[!UICONTROL 言語を追加]**」ボタンをクリックします。
   ...
   >[!IMPORTANT]
   >
   >**[!UICONTROL 言語を追加]** ボタンは、ターゲットディメンションに言語スキーマが含まれている場合にのみ使用できます。 スキーマとターゲットディメンションについて詳しくは、[ 詳細ドキュメント ](https://experienceleague.adobe.com/en/docs/campaign-web/v8/audiences/targeting-dimensions){target=_blank} を参照してください。
...
1. 追加する言語を **[!UICONTROL 言語]** ドロップダウンリストで選択します。 第 1 言語を追加するとデフォルトに設定され、現在のコンテンツがデフォルトのコンテンツになります。 新しい言語を追加する場合、コンテンツはデフォルトコンテンツに基づきます。
   ...
   >[!NOTE]
   >
   >このリストから使用できる言語は、言語属性で定義された値（system、user、dbenum などの値）によって異なります 列挙の管理について詳しくは、この [ 節 ](https://experienceleague.adobe.com/en/docs/campaign-web/v8/conf/enumerations){target=_blank} を参照してください。
