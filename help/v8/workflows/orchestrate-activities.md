...
ワークフローメニューまたはキャンペーン内で[ワークフローを作成](create-workflow.md)したら、実行される様々なタスクの調整を開始できます。 これを行うために、ワークフローダイアグラムを構築できる視覚的なキャンバスが用意されています。 このダイアグラムに様々なアクティビティを追加し、順番に接続できます。 カンバスの周りに水平スクロールバーと垂直スクロールバーが表示され、表示する領域に直接ドラッグして大きなワークフローを移動できます。
...
アクティビティを削除するには、キャンバスでアクティビティを選択し、アクティビティプロパティの「**削除**」アイコンをクリックします。 使用可能なオプションについては、[ アクティビティの削除と切断](#delete)を参照してください。
...
カンバスの右上隅にあるツールバーには、アクティビティを簡単に操作し、カンバス内を移動するためのオプションが用意されています。

* **複数選択モード**：複数のアクティビティを選択してすべてを一度に削除するか、コピー＆ペーストします。 詳しくは、[この節](#copy)を参照してください。
* **分岐を追加**：ツールバーの「**+**」ボタンをクリックして、キャンバス上に別の実行分岐を作成します。 結果は、並列パスに[分岐](activities/fork.md)を使用することと同じですが、図はグラフィカルにより明確になります。
...
* キャンバスからアクティビティを「**削除**」します。 詳しくは、[この節](#delete-activity)を参照してください。
...
## アクティビティの削除と切断 {#delete}

### アクティビティの削除 {#delete-activity}

アクティビティを削除するには、キャンバスでアクティビティを選択し、アクティビティプロパティの&#x200B;**削除** アイコンをクリックします。 確認ダイアログが表示されます。

* アクティビティが他のアクティビティに接続されていない場合は、アクティビティを削除することを確認します。

  ![簡単なアクティビティの削除](assets/workflow-delete.png)

* アクティビティが1つ以上の後続アクティビティに接続されている場合は、それらの処理方法を選択します。

  ![複数のアクティビティを削除](assets/workflow-delete2.png)

  * **後続のすべてのアクティビティを削除**：アクティビティと、同じパス上でそのアクティビティに続くすべてのアクティビティを削除します。
  * **このアクティビティのみを削除**：選択したアクティビティのみを削除し、残りのパスを再接続します。 このオプションは、アクティビティに後継者が1人いる場合にのみ使用できます。
  * **削除して新しいブランチを作成**：選択したアクティビティは削除されますが、後続のアクティビティは保持され、新しい別のブランチに移動します。

**削除**&#x200B;をクリックして選択を確定するか、**キャンセル**&#x200B;をクリックして何も削除せずにダイアログを閉じます。

### トランジションの切断 {#disconnect-transition}

どちらのアクティビティも削除せずに、2つのアクティビティを切断できます。 切断されたトランジションの後に配置されたアクティビティは削除されません。これらは、ワークフローの新しい別のブランチに移動されます。

これにより、ワークフローダイアグラムを再整理できます。例えば、保存するアクティビティのグループを一時的に保存し、削除して再作成する必要はありません。

これは、1つのトランジションで実行できます。

1. 切断するトランジションを選択します。

1. 移行プロパティの「**切断**」アイコンをクリックします。

   ![移行プロパティパネルの切断アイコン ](assets/workflow-transition.png)

   このアイコンは、トランジションが下流アクティビティに導く場合にのみ使用できます。 確認ダイアログが表示されます。

1. **切断**&#x200B;をクリックして確定するか、**キャンセル**&#x200B;をクリックして、何も切断せずにダイアログを閉じます。

   ![移行確認ダイアログの切断](assets/workflow-transition2.png)

ソースアクティビティに複数のアウトバウンドトランジションがある場合（例えば、複数の結果ブランチを持つ&#x200B;**Split** アクティビティや&#x200B;**Fork** アクティビティなど）、アクティビティの独自のプロパティペインから各アクティビティを個別に削除できます。

1. アクティビティを選択し、**セグメント** セクションで削除するトランジションを見つけます。

1. トランジションの横にあるゴミ箱アイコンをクリックします。 確認ダイアログが表示されます。

   セグメント結果の横にある![ごみ箱アイコン ](assets/workflow-transition3.png)

1. 「**削除**」をクリックして確定するか、「**キャンセル**」をクリックして何も削除せずにダイアログを閉じます。

   ![移行確認ダイアログの削除](assets/workflow-transition4.png)
