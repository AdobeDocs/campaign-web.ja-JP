...
>title="ワークフローのプロパティ"
>abstract="この画面で、ワークフローの作成に使用するテンプレートを選択し、ラベルを指定します。 を展開します。 **その他のオプション** ワークフローの内部名、そのフォルダー、タイムゾーン、スーパーバイザーグループなど、さらに多くの設定を行う場合は、「 」セクションを参照してください。 エラーが発生した場合にオペレーターに警告できるように、スーパーバイザーグループを選択することを強くお勧めします。"
...
The **[!UICONTROL プロパティ]** 「 」セクションには、ワークフローの作成時に設定できる汎用設定が用意されています。 既存のワークフローのプロパティにアクセスするには、 **[!UICONTROL 設定]** ワークフローキャンバスの上にあるアクションバーで使用できるボタン。
