---
audience: end-user
title: データ読み込み（RDBMS）ワークフローアクティビティを使用
description: データ読み込み（RDBMS）ワークフローアクティビティの使用方法について説明します。
...
# データ読み込み（RDBMS） {#data-loading-rdbms}
...
このオプションを使用すると、サーバーレベルで既に定義されているデータベース接続を使用できます。
...
1. 「**[!UICONTROL 抽出する列を追加]**」をクリックして、代わりに特定の列を収集するか、追加します。
...
## ソースフィルター（オプション） {#filter}

外部テーブルからデータの一部のみを収集するには、フィルターを定義します。

1. 「**[!UICONTROL ソースフィルター]**」セクションで、「**[!UICONTROL クエリを編集]**」をクリックします。
