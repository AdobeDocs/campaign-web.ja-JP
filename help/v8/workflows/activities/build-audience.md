...
1. 「**エンリッチメントデータ**」セクションを使用して、データベースからの追加情報（例：契約参照やニュースレター購読）でターゲットにするデータを強化します。 そのデータは、ワークフローの&#x200B;**作業用テーブル**&#x200B;のオーディエンスと共に保存され、後続のアクティビティで使用できます。 単一のエンリッチメント属性、コレクションリンク、またはエクスプレッションを追加し、詳細オプションにアクセスできます。 手順と例について詳しくは、[エンリッチメントデータの追加](enrichment.md#enrichment-add)を参照してください。
