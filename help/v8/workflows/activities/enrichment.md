...
>[!NOTE]
>
>**オーディエンスを作成** アクティビティ（クエリタイプ）も&#x200B;**エンリッチメントデータ**&#x200B;をサポートしています。 [詳細情報](build-audience.md#build-audience-configuration)を参照してください。
...
「**エンリッチメントデータ**」セクションは、**エンリッチメント**&#x200B;および&#x200B;**オーディエンスの構築** （クエリタイプ）アクティビティで利用できます。 契約参照やニュースレター購読など、データベースからの追加情報を使用して、ターゲットデータを強化できます。 そのデータは、ワークフロー&#x200B;**ワークテーブル**&#x200B;のオーディエンスと共に保存され、後のアクティビティで使用できます。 単一のエンリッチメント属性、コレクションリンク、またはエクスプレッションを追加し、詳細オプションにアクセスできます。
...
>[!NOTE]
>
>利用できるオプションは、**オーディエンスを作成** アクティビティと&#x200B;**エンリッチメント** アクティビティで異なります。

アクティビティごとに次のオプションを使用できます。

+++ オーディエンスアクティビティの作成（クエリタイプ）

* **[!UICONTROL メインセットからのすべての追加データを保持]**：出力トランジションのメインインバウンドセットからの追加の列を保持します。
* **[!UICONTROL ターゲットディメンション要素でデータをグループ化]**：結果をグループ化して、ターゲットレコードが1回だけ表示されるようにします。
* **[!UICONTROL 重複行を削除（DISTINCT）]**：結果セットから重複行を削除します。
* **[!UICONTROL ターゲティングディメンションのプライマリキーの自動追加を無効にする]**: アクティビティがターゲティングディメンションのプライマリキーを自動的に結果に追加できないようにします。
* **[!UICONTROL 0個のID レコードの自動フィルタリングを無効にする]**：識別子の値が0のレコードを自動的にフィルタリングせずに保持します。
* **[!UICONTROL インバウンドイベントデータを使用]**: インバウンドトランジションのデータをアクティビティの作業入力として使用します。
...
* **[!UICONTROL 重複行を削除（DISTINCT）]**：結果セットから重複行を削除します。
* **[!UICONTROL 0個のID レコードの自動フィルタリングを無効にする]**：識別子の値が0のレコードを自動的にフィルタリングせずに保持します。
* **[!UICONTROL 結果の各行に識別子を追加]**：各出力行に一意の識別子を追加します。
...
1. 配信アクティビティを開き、コンテンツ編集に移動します。「**[!UICONTROL オファー設定]**」ボタンをクリックし、ドロップダウンリストで、オファーに対応する&#x200B;**[!UICONTROL オファースペース]**を選択します。
エンリッチメントアクティビティのオファーのみを表示する場合は、**[!UICONTROL 提案]**&#x200B;の数を0に設定し、変更を保存します。
