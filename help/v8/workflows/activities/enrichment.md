...
>[!NOTE]
>
>**オーディエンスを作成**&#x200B;アクティビティ（クエリタイプ）は、**エンリッチメントデータ**&#x200B;もサポートしています。詳しくは、[こちら](build-audience.md#build-audience-configuration)を参照してください。
...
**エンリッチメント**&#x200B;アクティビティと&#x200B;**オーディエンスを作成**&#x200B;アクティビティ（クエリタイプ）では、「**エンリッチメントデータ**」セクションを使用できます。これにより、データベースからの追加情報（例：契約参照やニュースレターの購読）を使用して、ターゲットデータを強化できます。そのデータは、ワークフローの&#x200B;**作業用テーブル**&#x200B;のオーディエンスと共に保存され、後続のアクティビティで使用できます。単一のエンリッチメント属性、コレクションリンクまたは式を追加し、高度なオプションにアクセスできます。

「**エンリッチメントデータを追加**」をクリックし、エンリッチメントに対して使用する属性を選択します。 [詳しくは、属性を選択してお気に入りに追加する方法を参照してください](../../get-started/attributes.md)。
...
1 つ以上のエンリッチメント属性を追加した後、「**[!UICONTROL 詳細設定パラメーター]**」をクリックして、エンリッチメントデータの作成方法（グループ化、重複排除 - 重複、プライマリキーの処理、インバウンドイベントデータなど）を設定します。これらのオプションは、クライアントコンソールをミラーリングし、高度なワークフローシナリオを対象としています。

![エンリッチメントの詳細設定パラメーターを示すスクリーンショット](../assets/workflow-query-advanced-parameters.png)

>[!NOTE]
>
>使用可能なオプションは、**オーディエンスを作成**&#x200B;アクティビティと&#x200B;**エンリッチメント**&#x200B;アクティビティで異なります。

各アクティビティで、次のオプションを使用できます。

+++ オーディエンスを作成アクティビティ（クエリタイプ）

* **[!UICONTROL メインセットからのすべての追加データを維持]**：メインのインバウンドセットからの追加の列を出力トランジションで保持します。
* **[!UICONTROL ターゲティングディメンション要素でデータをグループ化]**：各ターゲットレコードが 1 回のみ表示されるように、結果をグループ化します。
* **[!UICONTROL 重複行を削除（DISTINCT）]**：結果セットから重複行を削除します。
* **[!UICONTROL ターゲティングディメンションのプライマリキーの自動追加を無効にする]**：アクティビティで結果にターゲティングディメンションのプライマリキーを自動的に追加するのを防ぎます。
* **[!UICONTROL 0 ID レコードの自動フィルターを無効にする]**：識別子の値が 0 のレコードを自動的に除外する代わりに保持します。
* **[!UICONTROL インバウンドイベントデータを使用]**：インバウンドトランジションからのデータをアクティビティの作業用入力として使用します。
...
* **[!UICONTROL 重複行を削除（DISTINCT）]**：結果セットから重複行を削除します。
* **[!UICONTROL 0 ID レコードの自動フィルターを無効にする]**：識別子の値が 0 のレコードを自動的に除外する代わりに保持します。
* **[!UICONTROL 結果の各行に識別子を追加]**：各出力行に一意の ID を追加します。
