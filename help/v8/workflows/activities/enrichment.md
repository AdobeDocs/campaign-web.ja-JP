...
1. クリック **エンリッチメントデータを追加** をクリックし、データのエンリッチメントに使用する属性を選択します。

   ターゲットディメンションから[単一エンリッチメント属性](#single-attribute)または[コレクションリンク](#collection-link)の 2 種類のエンリッチメントデータを選択できます。

   >[!NOTE]
   >
   >The **式を編集ボタン** 属性選択画面で、高度な式を作成して属性を選択できます。 [式エディターの使用方法を説明します。](../../query/expression-editor.md)
...
ここでは、エンリッチメント属性の最大値を定義します。100$を超える項目は除外されます。 [クエリモデラーの操作方法を説明します。](../../query/query-modeler-overview.md)
