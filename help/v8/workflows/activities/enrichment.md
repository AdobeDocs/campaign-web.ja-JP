...
例えば、このオプションを使用して、アップロードされたファイルで指定されたプロファイルの国を、Campaign データベースの専用テーブルで使用できる国の 1 つと紐付けることができます。

次の手順に従って、 **エンリッチメント** 紐付けリンクを使用したアクティビティ :

1. 「」をクリックします **リンクを追加** のボタン **紐付け** セクション。
1. 紐付けリンクの作成対象となるデータを特定します。

   * Campaign データベースのデータと紐付けリンクを作成するには、以下を選択します。 **データベーススキーマ** ターゲットが格納されているスキーマを選択します。
   * 入力トランジションからのデータに基づいて紐付けリンクを作成するには、以下を選択します。 **一時スキーマ** ターゲットデータが格納されるワークフロートランジションを選択します。

1. この **ラベル** および **名前** フィールドは、選択したターゲットスキーマに基づいて自動的に入力されます。 必要に応じて、値を変更できます。

1. が含まれる **紐付け条件** セクションで、ソース表と宛先表からのデータの紐付け方法を指定します。

   * **単純結合**：ソーステーブルの特定のフィールドを宛先テーブルの別のフィールドと紐付けます。 それには、 **結合を追加** ボタンをクリックし、 **ソース** および **宛先** 紐付けに使用するフィールド。

     >[!NOTE]
     >
     >1 つ以上を使用できます **単純結合** 条件（この場合、データを一緒にリンクできるように、それらをすべて検証する必要があります）。

   * **高度な結合**：クエリモデラーを使用して、紐付け条件を設定します。 それには、 **条件を作成** 「」ボタンをクリックし、AND および OR 操作を使用して独自のルールを作成して、紐付け条件を定義します。

以下の例は、Adobe Campaign データベースの受信者テーブルと、で生成された一時テーブルの間にリンクを作成するように設定されたワークフローを示しています。 **ファイルをロード** アクティビティ。 この例では、エンリッチメント アクティビティが、メールアドレスを紐付け条件として使用して両方のテーブルを紐付けします。
...
次の例に、2 つのトランジション間にリンクを作成するように設定されたワークフローを示します。最初のトランジションは、を使用してプロファイルデータをターゲットにします **クエリ** アクティビティに対して、2 番目のトランジションには、ファイルを読み込みアクティビティを通じて読み込まれるファイルに保存された購入データが含まれます。

![](../assets/enrichment-uc-link.png)

* 第 1 **エンリッチメント** アクティビティは、プライマリセットにリンクします（データは **クエリ** activity）に設定し、そのスキーマを **ファイルをロード** アクティビティ。 これにより、クエリのターゲットになる各プロファイルを、対応する購入データと一致させられるようになります。
