...
1. ワークフローに複数のトランジションが設定されている場合は、 **[!UICONTROL プライマリセット]** データを充実させるために、どのトランジションをプライマリセットとして使用するかを定義するフィールド。

## エンリッチメントデータを追加 {#enrichment-add}

1. 「**エンリッチメントデータを追加**」をクリックし、データのエンリッチメントに使用する属性を選択します。

   ターゲットディメンションから単一エンリッチメント属性またはコレクションリンクの 2 種類のエンリッチメントデータを選択できます。 これらの各タイプについて、以下の例で詳しく説明します。
...
使用できるリンクには、次のようにいくつかのタイプがあります。

* **[!UICONTROL カーディナリティ 1 のシンプルリンク]**：プライマリセットの各レコードを、リンクされたデータの 1 つのレコードにのみ関連付けることができます。
* **[!UICONTROL カーディナリティ 0 または 1 のシンプルリンク]**：プライマリセットの各レコードは、リンクされたデータの 0 または 1 つのレコードに関連付けることができますが、複数のレコードには関連付けることができません。
* **[!UICONTROL カーディナリティ N のコレクションリンク]**：プライマリセットの各レコードは、リンクされたデータの 0、1 または複数（N）のレコードに関連付けることができます。

リンクを作成するには、次の手順に従います。

1. が含まれる **[!UICONTROL リンク定義]** セクションで、 **[!UICONTROL リンクを追加]** ボタン。

   ![](../assets/workflow-enrichment-link.png)

1. が含まれる **関係タイプ** ドロップダウンリストから、作成するリンクのタイプを選択します。

1. プライマリセットをリンクするターゲットを特定します。

   * データベース内の既存のテーブルをリンクするには、 **[!UICONTROL データベーススキーマ]** から目的のテーブルを選択します。 **[!UICONTROL ターゲットスキーマ]** フィールド。
   * 入力トランジションのデータとリンクするには、次を選択します **一時スキーマ** 使用するデータのトランジションを選択します。

1. プライマリセットのデータをリンクされたスキーマと照合するための紐付け条件を定義します。 次の 2 種類の結合を使用できます。

   * **単純結合**:2 つのスキーマのデータに一致する特定の属性を選択します。 クリック **結合を追加** を選択し、 **ソース** および **宛先** 紐付け条件として使用する属性。
   * **高度な結合**：詳細設定を使用して結合を作成します。 クリック **結合を追加** をクリックし、 **条件を作成** ボタンをクリックしてクエリモデラーを開きます。

リンクを使用したワークフローの例は、次の場所で入手できます [例](#link-example) セクション。
...
### リンクデータによるエンリッチメント {#link-example}

次の例に、2 つのトランジション間にリンクを作成するように設定されたワークフローを示します。 第 1 のトランジションは、クエリアクティビティを使用してプロファイルデータをターゲットにし、第 2 のトランジションは、ファイルを読み込みアクティビティを介して読み込まれるファイルに保存された購入データを含む。

* 第 1 **エンリッチメント** アクティビティは、プライマリセットをリンクします（のデータ **クエリ** activity）に設定し、そのスキーマを **ファイルをロード** アクティビティ。 これにより、クエリのターゲットとなる各プロファイルを、対応する購入データと照合できます。
* 秒 **エンリッチメント** アクティビティを追加して、からの購入データでワークフローテーブルのデータを強化します。 **ファイルをロード** アクティビティ。 これにより、これらのデータをさらなるアクティビティで使用して、例えば、購入に関する情報を含む顧客に送信されるメッセージをパーソナライズすることができます。
