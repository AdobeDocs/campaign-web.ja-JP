...
>[!NOTE]
>サポートされるファイル形式は、テキスト（TXT）とコンマ区切り値（CSV）です。Web ユーザーインターフェイスコンソールを使用すると、最大 50 MB のファイルを読み込むことができます。 クライアントコンソールのデータ読み込みアクティビティの制限は 150 MB です。 [詳細情報](https://experienceleague.adobe.com/docs/campaign-web/v8/wf/design-workflows/load-file.html?lang=ja){target="_blank"}
