...
>abstract="**重複排除 - 重複**&#x200B;アクティビティでは、インバウンドアクティビティの結果から重複を削除します。 主に、ターゲティングアクティビティの後と、ターゲティングデータを使用するアクティビティの前に使用されます。 複数のインバウンドトランジションが使用可能な場合は、「**結合に設定**」セクションを使用して、アクティビティに接続するトランジションを選択します。"

>[!CONTEXTUALHELP]
>id="acw_orchestration_deduplication_sets"
>title="結合の設定"
>abstract="**重複排除** アクティビティのインバウンドトランジションとして接続する以前のアクティビティを確認します。 次に、選択したアクティビティが&#x200B;**重複排除**&#x200B;に接続されます。 このセクションは、複数のインバウンドトランジションがアクティビティに接続できる場合にのみ表示されます。"
...
このアクティビティは、複数のインバウンド移行をサポートしています。 複数のインバウンドトランジションが使用可能な場合は、アクティビティプロパティの「**結合に設定**」セクションを使用して、アクティビティに接続するトランジションを選択します。 選択したトランジションは、ワークフローキャンバスの&#x200B;**重複排除**&#x200B;にリンクされます。
...
1. **結合に設定** セクションで、**重複排除** アクティビティのインバウンドトランジションとして接続する前のアクティビティを確認します。 選択したアクティビティは、ワークフローキャンバスの&#x200B;**重複排除**&#x200B;にリンクされます。 参照インバウンドトランジションを定義するには、**プライマリセット** フィールドを使用します。 他のセットのレコードは、プライマリセットと照合され、重複が識別されます。

   >[!NOTE]
   >
   >このセクションは、複数のインバウンドトランジションが使用可能な場合にのみ表示されます。
