...
「チャネル」アクティビティを挿入する前に、関連するアクティビティを使用してワークフローの作成を開始します。

* 配信アクティビティを挿入する前に、オーディエンスを定義する必要があります。 オーディエンスは、配信の主なターゲットである、メッセージを受信するプロファイルです。 キャンペーンワークフローのコンテキストでメッセージを送信する場合、メッセージオーディエンスは、チャネルアクティビティでは定義されず、次のような専用のアクティビティ内で定義されます。
