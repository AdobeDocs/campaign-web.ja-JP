...
* [オーディエンスを作成](build-audience.md)：ターゲット母集団を定義します。 既存のオーディエンスを選択するか、クエリモデラーを使用して独自のクエリを定義します。 また、**エンリッチメントデータ**&#x200B;を設定することもできます（クエリに使用できます）。
...
>title="終了アクティビティ"
>abstract="**開始**&#x200B;アクティビティを使用すると、ワークフローの開始をグラフィカルにマークできます。 このアクティビティは、機能上の影響はないので、省略可能です。"
...
* [結合](join.md): ワークフローの複数の実行ブランチを同期します。
* [スケジューラー](scheduler.md)：ワークフローの開始日時をスケジュールします。
* **開始**&#x200B;および[終了](end.md): ワークフローの開始と終了をグラフィカルにマークします。 これらはオプションであり、単独では機能への影響はありません。

  >[!NOTE]
  >
  >**開始** アクティビティは、26.3 リリースより前に作成されたすべてのワークフローに自動的に追加されます。 デフォルトでは、新しいワークフローには挿入されません。
