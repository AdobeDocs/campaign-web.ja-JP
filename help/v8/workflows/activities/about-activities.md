...
* 「**開始**」および「[終了](end.md)」：ワークフローの開始と終了を視覚的に示します。 これらはオプションであり、単独では機能への影響はありません。

  >[!NOTE]
  >
  >**開始**&#x200B;アクティビティは、26.3 リリースより前に作成されたすべてのワークフローに自動的に追加されます。 このアクティビティはオプションであるため、デフォルトでは新しいワークフローには挿入されません。
