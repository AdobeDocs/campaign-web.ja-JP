...
配信されたメッセージの数は、配信品質の割合を反映しています。 次の理由により、100%になることはありません。アドレスや電話番号の一部が間違っている可能性や、E メールプロバイダーのスパムブロッカーがメッセージを拒否する可能性、配信品質の問題が発生する可能性があります。
...
合計開封数は、開封数が生成された個々の受信者の数に関係なく、メッセージが開封された合計回数を追跡することで計算されます。 この指標は E メールでのみ使用できます。
...
* 配信で少なくとも 1 回クリックしたユニークユーザーの数。 購読解除リンクおよび E メールミラーページへのリンクは除外されます。
...
受信者は、E メールコンテンツの専用の購読解除リンクを通じて、または SMS に STOP と返信して、E メールおよび SMS からオプトアウトできる必要があります。
...
Adobe Campaignプラットフォームによって送信された一部のメッセージは、宛先に到達しない可能性があります。 この問題は、ユーザーアドレスまたは電話に入力ミスがあった場合、受信者が E メールアドレスを変更した場合、またはメールボックスがいっぱいになった場合に発生する可能性があります。 メッセージをプロファイルに送信できない場合、リモートサーバーは自動的にエラーメッセージを Adobe Campaign に送信します。このエラーは、メールアドレス、電話番号、またはデバイスを強制隔離する必要があるかどうかを判断するために適合されます。

そのため、常にデータベースを確認して更新し、すべてのプロファイルがアクティブで実際のものであることを確認する必要があります。 配信エラーは、メッセージが配信されなかった理由に応じて、一時的または永続的（ソフトバウンスまたはハードバウンス）になる場合があります。
