...
左側のナビゲーションメニューのほとんどのリンクには、**配信**&#x200B;や&#x200B;**キャンペーン**&#x200B;のリストなど、オブジェクトのリストが表示されます。 これらのリスト画面の一部は読み取り専用です。 リスト表示をカスタマイズしたり、リストをフィルタリングしたり、リスト データをCSV ファイルとして書き出したりできます（以下で詳しく説明します）。
...
## リストデータの書き出し {#export-list}

トラッキングログを含む任意のリスト画面からデータを書き出すことができます。 リストを書き出すには、次の手順に従います。

1. 書き出すリストを開きます。
1. 表示される列を調整し、目的の検索またはフィルターを適用します。 書き出しでは、画面に表示される列と、アクティブな検索またはフィルターが考慮されます。
