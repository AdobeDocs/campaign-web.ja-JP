...
左側のナビゲーションメニューのほとんどのリンクには、**配信**&#x200B;や&#x200B;**キャンペーン**&#x200B;のリストなど、オブジェクトのリストが表示されます。 これらのリスト画面の一部は読み取り専用です。 以下の説明に従って、リストの表示をカスタマイズし、これらのリストをフィルタリングし、リストデータを CSV ファイルとしてエクスポートできます。
...
## リストデータのエクスポート {#export-list}

トラッキングログを含む任意のリスト画面からデータをエクスポートできます。リストをエクスポートするには、次の手順に従います。

1. エクスポートするリストを開きます。
1. 表示される列を調整し、目的の検索またはフィルターを適用します。エクスポートでは、画面に表示されている列やアクティブな検索またはフィルターが考慮されます。
1. 必要に応じて、下にスクロールし、さらに行を表示します。現在リストに読み込まれている行のみがエクスポートされます。
1. リストの上にある「**読み込まれた行を CSV ファイルとしてエクスポート**」ボタンをクリックします。ファイルは、ブラウザーのデフォルトのダウンロードフォルダーに保存されます。

![スクリーンショットは、リストのエクスポートを示しています。](assets/filter-export.png){zoomable="yes"}
