...
>title="フィルターを保存"
>abstract="既存の定義済みフィルターを選択します。定義済みフィルターは、後で使用することを目的に作成および保存されるカスタムフィルターです。これらは、クエリモデラーを使用したフィルタリング操作中のショートカットとして機能します。これらは、例えば、データのリストをフィルタリングしたり、配信のオーディエンスを作成したりする際に使用できます。"
...
>title="フィルターを保存"
>abstract="この画面では、ルールを定義済みフィルターとして保存できます。新しい定義済みフィルターを作成するか、既存のフィルターを上書きできます。フィルターをお気に入りとして保存するには、「**[!UICONTROL お気に入りフィルター]**」オプションをオンに切り替えます。また、「**[!UICONTROL 共有フィルター]**」オプションを有効にして、他のユーザーがフィルターにアクセスできるようにすることもできます。"
...
1. 「**新規フィルターを作成**」を選択して、そのフィルターの名前と説明を入力します。
   ...
   必要に応じて、フィルターをお気に入りに保存できます。詳しくは、[このセクション ](#fav-filter)を参照してください。 **[!UICONTROL 共有フィルター]** オプションを有効にして、フィルターを他のユーザーがアクセスできるようにすることもできます。 詳しくは、[こちら](#share-filter)を参照してください。
...
フィルターが定義済みフィルターに追加されます。必要に応じて、フィルターをお気に入りに保存できます。詳しくは、[このセクション ](#fav-filter)を参照してください。 **[!UICONTROL 共有フィルター]** オプションを有効にすることで、フィルターを他のユーザーと共有することもできます。 詳しくは、[こちら](#share-filter)を参照してください。
...
デフォルトでは、作成した定義済みフィルターはプライベートで、自分にしか表示されません。 フィルターを組織内の他のオペレーターがアクセスできるようにするには、**[!UICONTROL 共有フィルター]** オプションを有効にします。

![共有フィルターオプション ](assets/predefined-filters-shared.png){zoomable="yes"}

共有フィルターは、すべてのユーザーに対して事前に定義されたフィルターリストに表示され、独自のワークフローやクエリでこれらのフィルターを使用できます。
...
1. オーディエンスの作成、選択、確認に使用する定義済みフィルターを参照します。
   ...
   ![](../content/assets/do-not-localize/Smock_More_18_N.svg) ボタンを使用して、既存の定義済みフィルターを編集または削除することもできます。
...
また、定義済みフィルターをルールビルダーから直接編集または削除できます。定義済みフィルター選択ウィンドウから、「**その他のアクション**」ボタンを使用します。フィルターを削除または編集できます。ラベルを変更したり、説明を追加したり、お気に入りとして追加したり、**[!UICONTROL 共有フィルター]** オプションを切り替えたりして、フィルターにアクセスできるユーザーを制御できます。
