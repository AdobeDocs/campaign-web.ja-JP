...
1. 「**[!UICONTROL 抽出形式]**」セクションを展開して、抽出ファイルの形式に関連する特定のオプションにアクセスします。 使用できる値は、選択した形式によって異なります。
   ...
   * **[!UICONTROL エンコード]**：抽出ファイルのエンコーディングを選択します。 デフォルトのエンコードは **Latin-1（1252）**&#x200B;です。 CJK 言語（日本語、韓国語、簡体字中国語、繁体字中国語）の受信者をターゲットにするダイレクトメール配信を作成する場合は、代わりに「**UTF-8（65001）**」を選択します。 Latin-1 では CJK 文字を表すことができないので、ファイルのプレビューが空になります。
