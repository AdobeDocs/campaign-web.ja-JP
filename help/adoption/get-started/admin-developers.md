...
* **サブドメイン設定** - Adobe Campaignで使用するドメインのサブセクション（技術的には「DNS ゾーン」）を設定できます。
...
詳しくは、[&#x200B; ブランディングドキュメント &#x200B;](../../v8/administration/branding/branding-gs.md)を参照してください
...
Adobe Campaign Standard と同様に、Adobe Campaign v8 に含まれているワークフローモジュールを使用すると、アプリケーションサーバーの様々なモジュールにわたり、すべての範囲のプロセスとタスクを調整できます。 この包括的なグラフィック環境では、セグメンテーション、キャンペーンの実行、ファイル処理、人間の参加などのプロセスを設計できます。ワークフローエンジンは、これらのプロセスを実行および追跡します。 Campaign v8でのワークフローの開始方法については、[このドキュメント &#x200B;](https://experienceleague.adobe.com/ja/docs/campaign/campaign-v8/data/workflows){target="_blank"}を参照してください。
...
* お使いのCampaign Standard環境が&#x200B;**Adobe Experience Manager**&#x200B;と統合されている場合、Adobeでは&#x200B;**Adobe Experience Manager as a Cloud Service**&#x200B;に移行することをお勧めします。これにより、Campaign Web ユーザーインターフェイスで電子メールをデザインする際にこの機能を使用し、Adobe Experience Manager環境内で電子メール配信コンテンツとフォームの効率的な管理を促進できます。詳しくは、[このページ &#x200B;](../../v8/integrations/aem-content.md)を参照してください。
